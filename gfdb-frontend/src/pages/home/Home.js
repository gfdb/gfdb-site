...
const Home = () => {
	...
	useEffect(() => {
		...
		for (let i = 0; i < MY_NAME.length; i++) {
			...
			letters.push(letter_body)
		}
		World.add(engine.world, [ceiling, floor, wall_left, wall_right, ...letters])
		...
		if (constraints) {
			...
			for (let i = 0; i < MY_NAME.length; i++) {
				...
				*/
				...
				let letter_body = scene.engine.world.bodies[cntr]
